import './Redeem.css'

function Redeem(){
    return(
        <section className="mt-5 px-5">
            <div className="flex justify-start max-md:justify-center">
                <p className="bg-[#4400A5] px-5 h-[40px] grid items-center rounded text-white max-md:w-full max-md:text-center">รางวัลที่ 1 รางวัลละ 6,000,000 บาท</p>
            </div>
            <div className="bg-[#F0F0F07A] grid">

                <div className="flex gap-2 justify-center text-center items-center p-5 ">
                    <p className="custom-box">3</p>
                    <p className="custom-box">3</p>
                    <p className="custom-box">3</p>
                    <p className="custom-box">3</p>
                    <p className="custom-box">3</p>
                    <p className="custom-box">3</p>
                </div>
                <div className="flex gap-5 justify-center max-md:block">
                    <div className="grid text-center shadow-md max-md:my-5">
                        <div className="grid justify-center items-center text-center rounded bg-[#4400A5]">
                            <p className='text-white px-5 py-2'>เลขหน้า 3 ตัว <br />2 รางวัลๆละ 4,000 บาท</p>
                        </div>
                        <p className='bg-white rounded-none border-none p-5'>058 410</p>
                    </div>
                    <div className="grid text-center shadow-md max-md:my-5">
                        <div className="grid justify-center items-center text-center rounded bg-[#4400A5]">
                            <p className='text-white px-5 py-2'>เลขท้าย 3 ตัว <br />2 รางวัลๆละ 4,000 บาท</p>
                        </div>
                        <p className='bg-white rounded-none border-none p-5'>058 410</p>
                    </div>
                    <div className="grid text-center shadow-md ml-5 max-md:ml-0 max-md:my-5">
                        <div className="grid justify-center items-center text-center rounded bg-[#4400A5]">
                            <p className='text-white px-5 py-2'>เลขท้าย 2 ตัว<br />1 รางวัลๆละ 2,000 บาท</p>
                        </div>
                        <p className='bg-white rounded-none border-none p-5'>45</p>
                    </div>
                </div>

            <div className="flex flex-col justify-center items-center my-5 shadow-md">
                <div className="bg-[#4400A5] rounded text-white flex justify-between items-center w-full px-5 py-2  max-md:block max-md:text-center">
                    <p>รางวัลข้างเคียงรางวัลที่ 1</p>
                    <p>2 รางวัลๆละ 100,000 บาท</p>
                </div>
                <p className='bg-white rounded-none border-none p-5 text-center w-full text-[2.5rem] max-md:text-[100%]'>356756 356758</p>
            </div>

            <div className="flex gap-10 justify-center max-md:block">
                    <div className="grid text-center shadow-md max-md:my-5 w-full">
                        <div className="grid justify-center items-center text-center rounded h-[60px] bg-[#4400A5]">
                            <p className='text-white px-5 py-2'>รางวัลที่ 2<br />5 รางวัลๆละ 200,000 บาท</p>
                        </div>
                        <p className='bg-white rounded-none border-none p-5'>830864 173448</p>
                        <p className='bg-white rounded-none border-none p-5'>095941 979994</p>
                        <p className='bg-white rounded-none border-none p-5'>752506</p>
                    </div>
                    <div className="grid text-center shadow-md max-md:my-5 w-full">
                        <div className="grid justify-center items-center text-center rounded h-[60px] bg-[#4400A5]">
                            <p className='text-white px-5 py-2'>รางวัลที่ 2<br />5 รางวัลๆละ 200,000 บาท</p>
                        </div>
                        <div className="flex justify-center items-center bg-white  rounded-none border-none p-5 mt-[-6rem] max-md:mt-0">
                            <p>663106 081787 543436 087303 630453</p>
                        </div>
                    </div>
            </div>

            <div className="grid w-full my-5 shadow-md">
                <div className="grid justify-center items-center text-center rounded h-[60px] bg-[#4400A5]">
                    <p className='text-white px-5 py-2'>รางวัลที่ 4    50 รางวัลๆละ 40,000 บาท</p>
                </div>
                <div className="table-contents">
                    <table class="striped-table border-x-0">
                        <tbody>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>

            <div className="grid w-full my-5 shadow-md">
                <div className="grid justify-center items-center text-center rounded h-[60px] bg-[#4400A5]">
                    <p className='text-white px-5 py-2'>รางวัลที่ 5    100 รางวัลๆละ 20,000 บาท</p>
                </div>
                <div className="table-contents">
                    <table class="striped-table border-x-0">
                        <tbody>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                            <tr>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                                <td>000000</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>

            </div>
        </section>
    );
}
export default Redeem